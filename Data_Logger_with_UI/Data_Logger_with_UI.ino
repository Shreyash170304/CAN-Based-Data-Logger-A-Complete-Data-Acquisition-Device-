...
// ESP32-C6 I2C bus (shared with RTC/IMU) on default pins for Smartelex board
TwoWire &WireRTC = Wire;       // Use default Wire instance (pins set in setup)
#include <WiFi.h>              // WiFi library for ESP32-C6
#include <WebServer.h>         // Web server for file access
#include <cstring>

// Neopixel LED configuration (match CAN_Data_Logger_Encoded.ino)
#include <Adafruit_NeoPixel.h>
#define NEOPIXEL_PIN   8
#define NEOPIXEL_COUNT 1
Adafruit_NeoPixel pixels(NEOPIXEL_COUNT, NEOPIXEL_PIN, NEO_GRB + NEO_KHZ800);

// LED Status Colors (same palette as encoded logger)
#define LED_RED       pixels.Color(255, 0, 0)
#define LED_BLUE      pixels.Color(0, 0, 255)
#define LED_GREEN     pixels.Color(0, 255, 0)
#define LED_YELLOW    pixels.Color(255, 255, 0)
#define LED_CYAN      pixels.Color(0, 255, 255)
#define LED_MAGENTA   pixels.Color(255, 0, 255)
#define LED_ORANGE    pixels.Color(255, 165, 0)
#define LED_WHITE     pixels.Color(255, 255, 255)
#define LED_OFF       pixels.Color(0, 0, 0)
...
// Pin definitions for ESP32-C6 (Smartelex) - matched to CAN_Data_Logger_Only.ino
#define CAN_TX		17
#define CAN_RX		16
...
// RTC I2C bus (shared Wire instance)
#define RTC_SDA_PIN    22      // I2C SDA pin for RTC (ESP32-C6)
#define RTC_SCL_PIN    23      // I2C SCL pin for RTC (ESP32-C6)
...
// Initialize RTC module on shared I2C bus (pins 22/23)
bool initializeRTC() {
    Serial.println("Initializing RTC on shared I2C bus (pins 22/23)...");
    
    // Initialize I2C bus on RTC pins (shared Wire instance)
    ...
    if (!rtc.begin(&WireRTC)) {
        ...
        Serial.println("Check RTC wiring (SDA=pin 22, SCL=pin 23)");
...
// Disable RTC I2C bus (not strictly needed when sharing Wire; kept for compatibility)
void disableRTCI2C() {
    Serial.println("RTC I2C bus left enabled (shared Wire in use)");
}
...
void setup() {
    ...
    // ===== STEP 1: Initialize RTC on shared I2C bus (pins 22/23) =====
...
String getFormattedTime() {
    ...
    if (rtcAvailable) {
        // Use RTC for accurate time (use baseTime + elapsed to reduce I2C traffic)
...
void loop() {
    ...
    if(ESP32Can.readFrame(rxFrame, 5)) {
        ...
        if (rtcAvailable) {
            // Use baseTime + elapsed (avoid frequent RTC reads in loop)
            unsigned long elapsedSeconds = (millis() - bootMillis) / 1000;
